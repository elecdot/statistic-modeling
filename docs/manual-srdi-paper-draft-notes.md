...
`source_label_original` 用于审计。对于转载外省政策的来源站点记录，本文
使用 `configs/manual_srdi_jurisdiction_overrides_v1.csv` 进行人工复核修正，
使 `province` 表示政策实际辖区，而不是转载网站所在地。当前共有 15 条
此类修正。
...
> “新疆”。对于转载其他地区政策的来源站点记录，本文根据政策标题、
> 发文主体和来源信息对政策实际辖区进行人工复核修正。中央政策用于
> 总体政策文本描述和中央-地方对比，但不进入地方省份差异强度的计算。
