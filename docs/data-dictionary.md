...
The notebook
`notebooks/49c_manual_srdi_classified_record_tfidf_interpretability_v2.py`
adds an interpretability and quality-audit layer for the completed v2 MacBERT
classified records. It merges the row-level prediction table with the v2
full-text corpus, builds a reviewed domain-phrase vocabulary from the label-rule
keywords and full-text dictionary, and computes TF-IDF summaries within
high-confidence MacBERT label samples.

This is a paper-facing diagnostic layer only. It does not generate new DID
policy-intensity variables, does not change the classified record table, and
does not mutate the final policy-side panel.

Main outputs:

- `outputs/manual_srdi_macbert_tfidf_label_summary_v2.csv`
- `outputs/manual_srdi_macbert_tfidf_top_terms_v2.csv`
- `outputs/manual_srdi_macbert_tfidf_overlap_audit_v2.csv`
- `outputs/manual_srdi_macbert_tfidf_representative_docs_v2.csv`
- `outputs/manual_srdi_macbert_tfidf_interpretation_notes_v2.csv`
- `outputs/manual_srdi_macbert_fig_tfidf_top_terms_v2.png`
- `outputs/manual_srdi_macbert_fig_tfidf_label_heatmap_v2.png`

Key fields:

| Field | Type | Meaning |
| --- | --- | --- |
| `label` | string | MacBERT label interpreted by the TF-IDF diagnostic (`supply`, `demand`, `environment`, or `other`). |
| `analysis_rule` | string | Rule defining the label sample. Tool labels use high-confidence probabilities; `other` uses boundary/exclusion rules. |
| `term` | string | Reviewed domain phrase used in TF-IDF matching. |
| `mean_tfidf` | float | Mean TF-IDF score for the term inside the label sample. |
| `tfidf_lift` | float | Difference between within-label and out-of-label mean TF-IDF. |
| `label_doc_share` | float | Share of label-sample documents containing the term. |
| `source_categories` | string | Policy-tool category metadata inherited from the dictionary or label-rule vocabulary. |
| `policy_id` | string | Representative record identifier for manual inspection. |

### Manual SRDI Yearly TF-IDF Drift v2

The notebook `notebooks/49d_manual_srdi_yearly_tfidf_drift_v2.py` extends the
classified-record TF-IDF diagnostic to the `publish_year x label` level. It uses
the same reviewed domain-phrase vocabulary as the 49c interpretability notebook
and produces both all-records and local-only scopes.

This diagnostic is designed for policy-language evolution and quality audit,
especially checking whether the 2019 supplementary corpus is coherent with the
2020-2024 v2 corpus. It does not create new DID variables and does not alter the
final policy-side panel.

Main outputs:

- `outputs/manual_srdi_macbert_tfidf_year_label_summary_v2.csv`
- `outputs/manual_srdi_macbert_tfidf_top_terms_by_year_label_v2.csv`
- `outputs/manual_srdi_macbert_tfidf_yearly_drift_audit_v2.csv`
- `outputs/manual_srdi_macbert_tfidf_yearly_interpretation_notes_v2.csv`
- `outputs/manual_srdi_macbert_fig_tfidf_yearly_tool_terms_v2.png`
- `outputs/manual_srdi_macbert_fig_tfidf_yearly_drift_heatmap_v2.png`

Key fields:

| Field | Type | Meaning |
| --- | --- | --- |
| `scope` | string | Corpus scope for the diagnostic: `all_records` or `local_only`. |
| `publish_year` | integer | Policy publication year in the v2 2019-2024 window. |
| `label` | string | MacBERT label interpreted by the yearly TF-IDF diagnostic. |
| `high_confidence_doc_count` | integer | Number of high-confidence records in the `scope x year x label` cell. |
| `is_thin_cell` | boolean | Whether the cell has fewer than 10 high-confidence records and should be interpreted cautiously. |
| `term` | string | Reviewed domain phrase used in yearly TF-IDF matching. |
| `mean_tfidf` | float | Mean TF-IDF score for the term inside the `scope x year x label` cell. |
| `comparison_type` | string | Drift comparison type: annual-to-label-all-years or adjacent-year. |
| `drift_distance` | float | `1 - cosine_similarity`; larger values indicate stronger language drift. |
...
Final descriptive and QA artifacts are generated by
`notebooks/53b_did_ready_policy_intensity_panel_descriptive_qa_v2.py`:

- `outputs/manual_srdi_did_policy_intensity_panel_desc_stats_v2.csv`
- `outputs/manual_srdi_did_policy_intensity_panel_year_trend_v2.csv`
- `outputs/manual_srdi_did_policy_intensity_panel_province_ranking_v2.csv`
- `outputs/manual_srdi_did_policy_intensity_panel_region_group_template_v2.csv`
- `outputs/manual_srdi_did_policy_intensity_panel_region_summary_v2.csv`
- `outputs/manual_srdi_did_policy_intensity_panel_correlations_v2.csv`
- `outputs/manual_srdi_did_policy_intensity_panel_structure_correlations_v2.csv`
- `outputs/manual_srdi_did_policy_intensity_panel_outlier_audit_v2.csv`
- `outputs/manual_srdi_did_policy_intensity_panel_final_qa_v2.csv`
- `outputs/manual_srdi_did_policy_intensity_panel_handoff_notes_v2.csv`

The region and intensity-group template is a downstream heterogeneity-readiness
artifact only. It is not an estimated heterogeneous treatment effect.
